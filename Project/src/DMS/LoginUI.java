...
public class LoginUI extends JFrame {
    ...
    /**
     * ระบบค้นหาและโหลดรูปภาพ
     */
    private void loadImage() {
        java.net.URL imgURL = null;

        // 1. ค้นหาจาก Class Path หลัก (/img/content.png)
        imgURL = getClass().getResource("/img/" + IMAGE_NAME);

        // 2. ถ้าไม่เจอ ค้นผ่าน ClassLoader (img/content.png)
        if (imgURL == null) {
            imgURL = getClass().getClassLoader().getResource("img/" + IMAGE_NAME);
        }

        // 3. ถ้าไม่เจอ ค้นหาจาก root ของ ClassPath (/content.png)
        if (imgURL == null) {
        ...
        if (imgURL != null) {
            ...
            // 4. Fallback สำหรับตรวจค้นจาก File System ตอนรันใน IDE/Terminal
            ...
            if (localFile.exists()) {
                ...
                System.out.println("❌ ไม่พบไฟล์รูปภาพ! กรุณาตรวจสอบว่ามีไฟล์ '" + IMAGE_NAME + "' อยู่ในโฟลเดอร์ 'src/img/' หรือไม่");
            }
        }

        // ดักจับรอให้ภาพโหลดเข้า memory สมบูรณ์
